...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column("email", sa.Text, nullable=True, unique=False),  # use CITEXT for case-insensitive if desired
    ...
    op.create_table(
        ...
        sa.Column("is_supplement", sa.Boolean, nullable=False, server_default=sa.text("false")),
        ...
        sa.Column("carbohydrates", sa.Numeric(10, 2), nullable=False, server_default="0"),
    ...
    op.create_index("ix_meals_user_date_time", "meals", ["user_id", "date_entered", "meal_time"])
    ...
    op.create_table(
        ...
        sa.Column("id", sa.BigInteger, primary_key=True),
        sa.Column("user_id", sa.BigInteger, sa.ForeignKey("users.id", ondelete="CASCADE"), nullable=False),
        sa.Column("supplement_id", sa.BigInteger, sa.ForeignKey("supplements.id", ondelete="CASCADE"), nullable=False),
    ...
    op.create_index("ix_supp_entries_user_date_time", "supplement_entries", ["user_id", "date_consumed", "time_consumed"])
    ...
    op.create_index("ix_measurements_user_datetime", "measurements", ["user_id", "datetime"])
    ...
    op.create_table(
        ...
        sa.UniqueConstraint("user_id", "provider", "provider_user_id", name="uq_tracker_user_provider_uid"),
    )
    op.create_index("ix_tracker_user_primary", "tracker_accounts", ["user_id", "primary"])
...
def downgrade():
    for t in [
        "oauth_state","tracker_accounts","inventory","water","measurements",
        "supplement_entries","supplements","meals","profile","users"
    ]:
        op.drop_table(t)
