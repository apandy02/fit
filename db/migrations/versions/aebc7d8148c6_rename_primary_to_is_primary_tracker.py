...
def upgrade() -> None:
    ...
    op.create_index("ix_tracker_user_primary_tracker", "tracker_accounts", ["user_id", "is_primary_tracker"])


def downgrade() -> None:
    op.drop_index("ix_tracker_user_primary_tracker", table_name="tracker_accounts")
    op.alter_column('tracker_accounts', 'is_primary_tracker', new_column_name='primary')    # Recreate the original index
