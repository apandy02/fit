"""
Eval for LMPs deciphering kitchen inventory from user-provided descriptions and images.
"""
...
from fit.ai.nutrition.assistants import decipher_inventory
from fit.ai.nutrition.data_models import KitchenInventory, KitchenItem
...
def prepare_eval_data():
    ...
    for item in data["data"]:
        dataset.append(
            {
                "input": item["input"],
                "reference": KitchenInventory.model_validate(item["output"]),
            }
        )
...
def item_precision_metric_wrapper(prediction, reference):
    metrics = calculate_item_metrics(prediction, reference)
    return metrics["precision"]


def item_recall_metric_wrapper(prediction, reference):
    metrics = calculate_item_metrics(prediction, reference)
    return metrics["recall"]


def calculate_item_metrics(
    prediction: List[KitchenItem], reference: List[KitchenItem]
) -> Dict[str, float]:
...
if __name__ == "__main__":
    ...
    eval = ell.evaluation.Evaluation(
        ...
        dataset=dataset,
        metrics={
            "item_precision": item_precision_metric_wrapper,
            "item_recall": item_recall_metric_wrapper,
    ...
    result = eval.run(decipher_inventory)
    ...
    logging.info(
        "Average Category Accuracy:", result.results.metrics["category_accuracy"].mean()
    )
    logging.info(
        "Average Quantity Accuracy:", result.results.metrics["quantity_accuracy"].mean()
    )
    logging.info(
        "Average Unit Accuracy:", result.results.metrics["unit_accuracy"].mean()
    )
