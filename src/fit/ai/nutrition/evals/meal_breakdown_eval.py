    ...
    for _, row in data.iterrows():
        meal_breakdown = MealBreakdown(
            ...
            macronutrients=Macronutrients(
                ...
                fat=Fats(
                    total=row["fat"], saturated=row["saturated"], trans=row["trans"]
                ),
            ),
            ...
            conditional_nutrients=ConditionalNutrients(creatine=row["creatine"]),
        )
...
def macro_calorie_consistency_metric(
    prediction: MealBreakdown, reference: MealBreakdown
) -> float:
    """
    ...
    predicted_cals_from_macros = (
        prediction.protein * 4 + prediction.carbohydrates * 4 + prediction.fat * 9
    )
...
def basic_accuracy_metric(prediction: MealBreakdown, reference: MealBreakdown) -> float:
    ...
    ref = np.array(
        [reference.calories, reference.protein, reference.carbohydrates, reference.fat]
    )
...
if __name__ == "__main__":
    ...
    logging.info(
        "Average Macro Calorie Consistency:",
        result.results.metrics["macro_calorie_consistency"].mean(),
    )
    logging.info(
        "Average Ingredients Score:", result.results.metrics["ingredients_score"].mean()
    )
    logging.info(
        "Average Basic Accuracy Score:",
        result.results.metrics["basic_accuracy_score"].mean(),
    )
