...
def get_status(user_id: int = Depends(get_current_user_id)):
...
def complete_profile(body: ProfileCompletionRequest, user_id: int = Depends(get_current_user_id)):
...
def complete_measurements(body: MeasurementsCompletionRequest, user_id: int = Depends(get_current_user_id)):
    try:
...
def complete_dietary(body: DietaryCompletionRequest, user_id: int = Depends(get_current_user_id)):
...
def handle_activity_selection(body: ActivitySelectionRequest, user_id: int = Depends(get_current_user_id)):
    ok = database_service.update_profile({
        "user_id": user_id,
...
def handle_goals_selection(body: GoalsSelectionRequest, user_id: int = Depends(get_current_user_id)):
    ok = database_service.update_profile({
        "user_id": user_id,
