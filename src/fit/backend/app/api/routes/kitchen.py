...
def add_inventory_item(req: AddInventoryItemRequest, user_id: int = Depends(get_current_user_id)):
    try:
...
def add_inventory_items(req: AddInventoryItemsRequest, user_id: int = Depends(get_current_user_id)):
    try:
...
def delete_inventory_item(rowid: int, user_id: int = Depends(get_current_user_id)):
...
def generate_grocery_list(user_id: int = Depends(get_current_user_id)):
