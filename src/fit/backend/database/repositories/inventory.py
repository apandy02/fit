...
class InventoryRepository:
    ...
    def insert_inventory_item(
        ...
        sql = text(
            "INSERT INTO inventory (title, quantity, unit, category, user_id) VALUES (:t, :q, :u, :c, :uid)"
        )
        with self._engine.begin() as conn:
            conn.execute(
                sql,
                {"t": title, "q": quantity, "u": unit, "c": category, "uid": user_id},
            )

    def get_inventory(self, user_id: int) -> dict:
        sql = text(
            "SELECT id, title, quantity, unit, category FROM inventory WHERE user_id = :u"
        )
        with self._engine.connect() as conn:
            rows = conn.execute(sql, {"u": user_id}).fetchall()
        results: dict[str, list[dict]] = {
            category: [] for category in dm.KITCHEN_ITEM_CATEGORIES
        }
        for r in rows:
            results[r.category].append(
                {
                    "rowid": r.id,
                    "title": r.title,
                    "quantity": r.quantity,
